(function (global, factory) {
    ...
    (global = global || self, global.IAT = factory(global.jsBase64));
    ...
    ***************************************************************************** */
    ...
    var WsWrapper = /** @class */ (function () {
        ...
        WsWrapper.prototype.connect = function (WsOption) {
            if (this.ws === null || this.ws.readyState !== WebSocket.OPEN) {
                this.ws = new WebSocket(WsOption.url);
    ...
    var AudioHandler = /** @class */ (function () {
        function AudioHandler(audioOption) {
        ...
        AudioHandler.prototype.start = function () {
        ...
        // 录音阶段
        AudioHandler.prototype.record = function (callback) {
            var _this = this;
            if (navigator.mediaDevices) {
                navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
                    var context = new AudioContext(); // 创建音频上下文
                    var audioSource = context.createMediaStreamSource(stream); // 创建节点
                    var audioScriptNode = context.createScriptProcessor(0, 1, 1); // 用于使用js直接操作音频数据 buffer暂时取0，选取该环境最适合的缓冲区大小
                    ...
                    var audioProcess = function (e) {
                        ...
                        if (_this.recordStatus) {
                            var result = new Float32Array(e.inputBuffer.getChannelData(0)).buffer;
                    ...
                    var newError = genError(Error.NO_RESPONSE, error.message);
                    _this.onError(newError);
                });
            }
            else {
                var error = genError(Error.NOT_SUPPORTED_TYPE, 'navigator.mediaDevices is not supported');
                this.onError(error);
            }
        };
        // 重采样阶段
        AudioHandler.prototype.resample = function (buffer, sampleRate, callback) {
            this.resampleWorker.postMessage({
                command: 'record',
                buffer: new Float64Array(buffer),
                sampleRate: sampleRate
            });
            this.resampleWorker.addEventListener('message', function (e) {
                var buffer = e.data.buffer;
                var result = new Int16Array(buffer).buffer;
                callback(result);
            });
        };
        // 音频压缩阶段
        AudioHandler.prototype.speex = function (buffer) {
            var _this = this;
            var output = new Int8Array([]);
            var inData = new Int16Array(buffer);
            ...
            this.speexWorker.addEventListener('message', function (e) {
                if (e.data.command === 'encode') {
                    var buffer_1 = e.data.buffer;
                    var result = new Int8Array(buffer_1);
                    _this.onAudioChunk(result.buffer);
                }
            });
        };
        AudioHandler.prototype.handleBuffer = function (buffer, sampleRate) {
        ...
        AudioHandler.prototype.manage = function () {
    ...
    (function (IATStatus) {
        IATStatus["idle"] = "idle";
        IATStatus["sessionBegin"] = "ssb";
        IATStatus["audioWrite"] = "auw";
        IATStatus["getResult"] = "grs";
        IATStatus["sessionEnd"] = "sse"; // 会话结束
    })(IATStatus || (IATStatus = {}));
    ...
    var Iat = /** @class */ (function () {
        function Iat(iatOption) {
            this.status = IATStatus.idle;
            this.audioHandler = null;
            this.ws = new WsWrapper();
            this.onStart = iatOption.onStart || identity$1;
            this.onMessage = iatOption.onMessage || identity$1;
            this.onError = iatOption.onError || identity$1;
            this.onEnd = iatOption.onEnd || identity$1;
        }
        Iat.prototype.start = function (startOption) {
            var _this = this;
            ...
            var wsCallback = {
                url: startOption.url,
                onOpen: function () {
                    _this.send(startOption, iatPayload, IATStatus.sessionBegin);
                },
                onMessage: function (result) {
                    try {
                        var ret = JSON.parse(result.data);
                        _this.processServerMessage.call(_this, ret, startOption, iatPayload);
                    }
                    catch (ex) { }
                },
                onClose: function () {
                    _this.end(startOption, iatPayload);
                },
                onError: function (error) {
                    var errors = genError(Error.NO_RESPONSE, error);
        ...
        // 针对不同的阶段发送不同的请求
        Iat.prototype.send = function (startOption, iatPayload, type) {
            // SSB
            if (type === IATStatus.sessionBegin) {
                var rpcParam = __assign(__assign({}, startOption.iatParam), { cmd: this.status, svc: iatPayload.svc });
            ...
            if (iatPayload.sid) {
                ...
                if (type === IATStatus.audioWrite) {
                    ...
                    var audioLength = iatPayload.recorderBuffer.length * frameSize;
                    var str = '';
                    ...
                    iatPayload.audioData = jsBase64.Base64.btoa(str)(+iatPayload.synid + 1).toString();
                    var audioStatus = iatPayload.audioStatus;
                    ...
                    var rpcParam = __assign(__assign({}, commonParam), { synid: iatPayload.synid, audioStatus: audioStatus, audiolen: iatPayload.audiolen, data: iatPayload.audioData });
                    this.ws.send(genRPCMessage(rpcParam));
                ...
                if (type === IATStatus.getResult) {
                    var rpcParam = __assign(__assign({}, commonParam), { type: startOption.iatParam.type });
                    this.ws.send(genRPCMessage(rpcParam));
                    return;
                }
                // SSE
        ...
        // ws返回的结果处理过程
        Iat.prototype.processServerMessage = function (responseMessage, startOption, iatPayload) {
            var rpcResponse = responseMessage.result;
            if (!rpcResponse) {
                var error_1 = genError(Error.NO_RESPONSE, responseMessage);
                this.onError(error_1);
                return;
            }
            var cmd = rpcResponse.cmd, ret = rpcResponse.ret;
            if (ret !== 0) {
                this.end(startOption, iatPayload);
                var error_2 = genError(Error.RESPONSE_ERROR, responseMessage);
                this.onError(error_2);
                return;
            }
            if (cmd === IATStatus.sessionBegin) {
                iatPayload.sid = rpcResponse.sid;
                // 开始录音操作
                if (this.audioHandler) {
            ...
            if (cmd === IATStatus.audioWrite) {
                var recStatus = rpcResponse.recStatus; // recStatus 为5表示流程结束了
                var iatrst = rpcResponse.iatrst;
                var iatpgs = rpcResponse.pgs;
                // 表示此时还没有结束，可以输出识别内容
                if (recStatus === 0) {
            ...
            if (cmd === IATStatus.getResult) {
                if (this.status !== IATStatus.getResult) {
                    var error_3 = genError(Error.RESPONSE_ERROR, responseMessage);
                    this.onError(error_3);
                    return;
                }
                var resultStatus = rpcResponse.resultStatus; // 为5表示流程结束了
                var iatrst = rpcResponse.iatrst;
                var iatpgs = rpcResponse.pgs;
                if (resultStatus === 5) {
                    this.onMessage({ end: true, pgs: iatpgs, result: iatrst });
            ...
            var error = genError(Error.RESPONSE_ERROR, rpcResponse.ret);
            this.end(startOption, iatPayload);
            this.onError(error);
        };
        // 拿到处理后的录音结果
        Iat.prototype.processAudioMessage = function (startOption, iatPayload, buffer) {
        ...
        Iat.prototype.end = function (startOption, iatPayload) {
            if (this.audioHandler) {
                this.audioHandler.stop();
            }
        ...
        Iat.prototype.stop = function (startOption, iatPayload) {
            if (this.audioHandler) {
                this.audioHandler.stop();
            }
    ...
    //# sourceMappingURL=Iat.js.map

    return Iat;

})));
//# sourceMappingURL=IAT.umd.js.map
